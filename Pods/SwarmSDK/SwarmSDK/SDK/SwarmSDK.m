...
- (void)locationManager:(CLLocationManager *)manager didDetermineState:(CLRegionState)state forRegion:(CLRegion *)region
{
    ...
    else
    {
        ...
        if(state == CLRegionStateInside)
        {
            notification.alertBody = @"You're inside a Swarm store";
            [self.locationManager startRangingBeaconsInRegion:(CLBeaconRegion *)region];
        }
        else if(state == CLRegionStateOutside)
        {
...
-(void)onWhereAmIRequestCompleted:(NSData *)responseData withError:(NSError *)error
{
    ...
    if([json isKindOfClass:[NSArray class]] && [json count] == 0) {
        ...
        TraceLog(@"<<< swarmSDK - onWhereAmIServiceCompleted empty array");
        return;
    }
...
-(void)onWhatIsHereRequestCompleted:(NSData*)responseData withError:(NSError*)error
{
    ...
    if (parseError!=nil) {
        if ([self.whatIsHereNotificationDelegate respondsToSelector:@selector(onWhatIsHereNotificationReceived:withError:)]) {
            [self.whatIsHereNotificationDelegate onWhatIsHereNotificationReceived:[SWARMWhatIsHereInformation new] withError:parseError];
        }
...
//    if()
    ...
    for (SWARMLocationInformaion *loci in locations) {
        for (BLESCampaign *blesc in campaigns) {
            if ([[blesc locationId] intValue]==[[loci dbid] intValue]) {
